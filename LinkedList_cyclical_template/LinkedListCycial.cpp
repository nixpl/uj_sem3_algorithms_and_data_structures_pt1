...
public:
    ...
    {
        cell<T> *lastPtr = Last(); // to dodalem

        while (header != lastPtr)
        {
            cell<T> *buff = header;
            header = header->next;
            delete buff;
    ...
    {                            // (lub w przypadku pustej listy tworzy komórkę z elementem x w miejscu głowy listy)
        if (p == nullptr || header == nullptr)
        {
            ...
            else // to dodalem
            {
                ...
                lastPtr->next = header; // to dodalem
            }
        }
        else
        {
    ...
    {
        cell<T> *buffPtr = header;
        while (buffPtr->next != header)
        {
            buffPtr = Next(buffPtr);
        }
        return buffPtr;
    };
...
public:
    ...
    {
        cell_with_prev<T> *lastPtr = Last(); // to dodalem
